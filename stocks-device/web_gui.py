...
function showFourierChart(target,e){const stock=stocks.find(x=>x.symbol===target.dataset.symbol),k=target.dataset.k,chart=stock?.[`fft_chart_${k}`];if(!chart?.actual?.length||!chart?.fitted?.length)return;const w=494,h=210,p=14,all=[...chart.actual,...chart.fitted],lo=Math.min(...all.map(d=>d.value)),hi=Math.max(...all.map(d=>d.value)),span=hi-lo||1,maxX=Math.max(...all.map(d=>d.x)),xy=d=>({...d,x:p+d.x*(w-2*p)/Math.max(maxX,1),y:p+(hi-d.value)*(h-2*p)/span}),actual=chart.actual.map(xy),fitted=chart.fitted.map(xy),past=fitted.filter(d=>!d.forecast),future=[past[past.length-1],...fitted.filter(d=>d.forecast)].filter(Boolean),line=a=>a.map(d=>`${d.x.toFixed(1)},${d.y.toFixed(1)}`).join(' '),todayX=p+chart.today_x*(w-2*p)/Math.max(maxX,1),turn=fitted[fitted.length-1];tip.className='fourier-tip';tip.innerHTML=`<div class="tip-title">${esc(stock.symbol)} · K${k} inverse Fourier · ${esc(chart.turn_type)} in ${stock[`fft_next_turn_${k}`].toFixed(1)}d</div><svg width="100%" height="210" viewBox="0 0 494 210"><line x1="${todayX}" y1="8" x2="${todayX}" y2="202" stroke="#91a1b3" stroke-width="1" stroke-dasharray="3 4"/><polyline points="${line(actual)}" fill="none" stroke="#35d07f" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/><polyline points="${line(past)}" fill="none" stroke="#ff9f43" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/><polyline points="${line(future)}" fill="none" stroke="#ff9f43" stroke-width="3" stroke-dasharray="6 5" stroke-linejoin="round" stroke-linecap="round"/><circle cx="${turn.x}" cy="${turn.y}" r="6" fill="#ff9f43" stroke="#ffe0bd" stroke-width="2"/></svg><div style="display:flex;gap:14px;flex-wrap:wrap;font-size:11px;direction:ltr"><span style="color:#35d07f">● detrended closes</span><span style="color:#ff9f43">● K${k} reconstruction</span><span style="color:#ffb873">- - forecast to ${esc(chart.turn_type)}</span></div>`;tip.style.display='block';moveTip(e)}
