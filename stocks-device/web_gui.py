    ...
    for batch in chunks(symbols, 25):
        ...
        for symbol in batch:
            ...
            item = Analytics(
                ...
                best_fourier, fourier_models, zigzag, fit_curve_models(daily_frame)
                if daily_frame is not None else None,
            )
            results[symbol] = item
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif}
    ...
    .tablebox>table{border-collapse:collapse;min-width:3450px;width:100%;direction:ltr}.tablebox>table>thead{position:sticky;top:0;background:#192430;z-index:2}
...
document.querySelector('thead th:nth-child(5)').insertAdjacentHTML('afterend','<th class="sortable" data-sort="fit_quality">Fit Q</th><th class="sortable" data-sort="fit_model">Best Fit</th><th class="sortable" data-sort="fit_phase">Fit Phase</th><th class="sortable" data-sort="fit_next_turn">Fit Next Turn</th><th class="sortable" data-sort="fit_forecast_5d">Fit 5d</th><th class="sortable" data-sort="fit_agreement">Agreement</th><th>All Fits</th>');
...
function decorateFitRows(){document.querySelectorAll('#rows tr').forEach(row=>{if(row.querySelector('.fits-open'))return;const symbol=row.cells[0]?.textContent.trim(),x=stocks.find(item=>item.symbol===symbol);if(!x)return;const values=[x.fit_quality===null?'—':x.fit_quality.toFixed(3),x.fit_model?esc(x.fit_model)+' · '+x.fit_window+'d':'—',esc(x.fit_phase||'—'),x.fit_next_turn===null?'—':esc(x.fit_turn_type)+' '+x.fit_next_turn.toFixed(1)+'d',x.fit_forecast_5d===null?'—':(x.fit_forecast_5d>=0?'+':'')+x.fit_forecast_5d.toFixed(2)+'%',x.fit_agreement===null?'—':(x.fit_agreement*100).toFixed(0)+'%',x.curve_models?.length?'<button class="fits-open" data-symbol="'+esc(symbol)+'">Open Fits</button>':'—'];let anchor=row.cells[3];values.forEach(value=>{const cell=document.createElement('td');cell.innerHTML=value;anchor.after(cell);anchor=cell})})}
...
new MutationObserver(()=>{decorateFitRows();decorateTalkRows()}).observe(document.getElementById('rows'),{childList:true});
function curveSvg(model){const pts=model.chart||[],all=pts.flatMap(p=>[p.actual,p.fitted]).filter(Number.isFinite),lo=Math.min(...all),hi=Math.max(...all),span=hi-lo||1,w=720,h=230,p=18,x=v=>p+(v-pts[0].x)*(w-2*p)/Math.max(1,pts[pts.length-1].x-pts[0].x),y=v=>p+(hi-v)*(h-2*p)/span,line=(key,filter)=>pts.filter(filter).map((d,i)=>`${i?'L':'M'} ${x(d.x).toFixed(1)} ${y(d[key]).toFixed(1)}`).join(' '),actual=line('actual',d=>Number.isFinite(d.actual)),past=line('fitted',d=>!d.forecast),future=line('fitted',d=>d.forecast);return `<svg viewBox="0 0 ${w} ${h}"><path d="${actual}" fill="none" stroke="#35d07f" stroke-width="2.5"/><path d="${past}" fill="none" stroke="#4da3ff" stroke-width="2.5"/><path d="${future}" fill="none" stroke="#ff9f43" stroke-width="3" stroke-dasharray="7 5"/></svg>`}
function openFits(symbol){const stock=stocks.find(x=>x.symbol===symbol);if(!stock?.curve_models?.length)return;const cards=stock.curve_models.map(m=>`<section><h2>${esc(m.name)} · ${m.window}d <b>Q ${m.quality.toFixed(3)}</b></h2>${curveSvg(m)}<div class="metrics"><span>Validation NRMSE ${m.validation_nrmse.toFixed(4)}</span><span>Direction ${(m.direction_accuracy*100).toFixed(0)}%</span><span>Turn ${(m.turn_accuracy*100).toFixed(0)}%</span><span>Stability ${(m.stability*100).toFixed(0)}%</span><span>5d ${(m.forecast_5d_pct>=0?'+':'')+m.forecast_5d_pct.toFixed(2)}%</span><span>${esc(m.phase)} · ${m.next_turn_type?esc(m.next_turn_type)+' '+m.next_turn.toFixed(1)+'d':'no reliable turn'}</span></div></section>`).join('');const win=window.open('','_blank');if(!win)return;win.document.write(`<!doctype html><meta charset="utf-8"><meta name="robots" content="noindex"><title>${esc(symbol)} Curve Models</title><style>body{margin:0;padding:28px;background:#0b1117;color:#e9eef4;font:15px system-ui}main{max-width:1000px;margin:auto}p{color:#91a1b3}section{background:#121a23;border:1px solid #31506c;border-radius:16px;padding:18px;margin:16px 0}h2{font-size:18px}h2 b{float:right;color:#35d07f}.metrics{display:flex;gap:16px;flex-wrap:wrap;color:#b9c7d5}.metrics span{background:#192430;padding:7px 10px;border-radius:8px}svg{width:100%;height:260px;background:#132335;border-radius:10px}</style><main><h1>${esc(symbol)} · all curve fittings</h1><p>Green: actual reference prices · blue: fitted history · orange dashed: causal forecast. Each model shows its best adaptive window selected by walk-forward validation.</p>${cards}</main>`);win.document.close()}
document.getElementById('rows').addEventListener('click',e=>{const fits=e.target.closest('.fits-open');if(fits){openFits(fits.dataset.symbol);return}const button=e.target.closest('.talk-check');if(!button)return;const cell=button.closest('.talk-cell');if(cell)requestTalk(cell.dataset.symbol)});
...
'Active cycles':'מספר המחזורים המשוער שעבר מאז Active Since; זה אינו מספר מחזורים שנצפו בפועל.','Regime':'active מעל 0.60, weakening בין 0.35 ל-0.60, broken מתחת 0.35.','Fit Q':'ציון תחזית מחוץ למדגם של מודל העקומה הטוב ביותר. משלב שגיאה, כיוון, נקודות מפנה, יציבות וכיסוי.','Best Fit':'המודל וחלון הזמן שניצחו ב-walk-forward עבור המניה.','Fit Phase':'הכיוון והקרבה לנקודת מפנה לפי מודל העקומה הנבחר.','Fit Next Turn':'נקודת המפנה הקרובה שמודל העקומה חוזה, אם נמצאה בטווח התחזית.','Fit 5d':'שינוי המחיר באחוזים שהמודל הנבחר חוזה לחמישה ימי מסחר.','Agreement':'אחוז המודלים המסכימים על כיוון התחזית לחמישה ימים.','All Fits':'פתיחת חלון חדש המציג את כל המודלים, הגרפים ותוצאות ה-validation.','Index':'המדד שבו המניה נכללת.'};
    ...
    def payload(self) -> dict[str, object]:
        with self.lock:
            ...
            for symbol in self.symbols:
                ...
                curves = analytics.curves if analytics else None
                best_fit = curves.get("best") if curves else None
                row.update({
                    "fit_quality": best_fit.get("quality") if best_fit else None,
                    "fit_model": best_fit.get("name") if best_fit else None,
                    "fit_window": best_fit.get("window") if best_fit else None,
                    "fit_phase": best_fit.get("phase") if best_fit else None,
                    "fit_next_turn": best_fit.get("next_turn") if best_fit else None,
                    "fit_turn_type": best_fit.get("next_turn_type") if best_fit else None,
                    "fit_forecast_5d": best_fit.get("forecast_5d_pct") if best_fit else None,
                    "fit_agreement": curves.get("agreement") if curves else None,
                    "curve_models": curves.get("models", []) if curves else [],
                })
                rows.append(row)
